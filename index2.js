  ...
  if (page_num_city % 5 != 0) {
    ...
    request(url, (err, response, html) => {
      ...
      let emails_arr = $('.email').map(() => {
         // return $(this).text();
         return { email: $(this).text(), data_id: $(this).attr("data-profile-id")}
      }).get();

      let addresses_arr = $('.address').map(() => {
         // return $(this).text();
         return { address: $(this).text(), data_id: $(this).attr("data-profile-id")}
      }).get();

      let names_arr = $('.name').map(() => {
         // return $(this).text();
         return { name: $(this).text(), data_id: $(this).attr("data-profile-id")}
      }).get();
      ...
      for (let i = 0; i < len; i++) {
        let data_id = emails_arr[i].data_id;

        let address = addresses_arr.find((i, el) => {
          return element.data_id == data_id;
        });
        if (address == undefined) { address = "" }

        let name = names_arr.find((i, el) => {
          return element.data_id == data_id;
        });
        if (name == undefined) { name = ""}

        let obj = { email: emails_arr[i], address, name, data_id, city: city, state: "IL", email_attempts: 0, has_responded: false, has_matched: false, requested_off: false };
